...
function __init__()
    # Resolve the native-library directory at RUNTIME, not at build time. @__DIR__ is frozen
    # into a sysimage/app when it is compiled, so a baked absolute path breaks the moment the
    # install is relocated (different machine, different folder). The host sets BIFRR_LIBDIR to
    # the real path on THIS machine; we fall back to @__DIR__ for plain (non-sysimage) use where
    # the source really is at that path. Paths stay absolute so each DLL's sibling dependencies
    # (roadrunner.dll, msvcp140.dll, ...) resolve from its own directory as before.
    libdir = get(ENV, "BIFRR_LIBDIR", @__DIR__)
    @static if Sys.iswindows() #windows 64
        global rrlib  = Libdl.dlopen(joinpath(libdir, "roadrunner_c_api.dll"))
        global antlib = Libdl.dlopen(joinpath(libdir, "libantimony.dll"))
    elseif Sys.islinux() #ubuntu
        global rrlib  = Libdl.dlopen(joinpath(libdir, "libroadrunner_c_api.so"))
        global antlib = Libdl.dlopen(joinpath(libdir, "libantimony.so"))
    #elseif Sys.isapple() #mac
    #    global rrlib = Libdl.dlopen(joinpath(libdir, "libroadrunner_c_api.dylib"))
    #    global antlib = Libdl.dlopen(joinpath(libdir, "libantimony.dylib"))
    else
