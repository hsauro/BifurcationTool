...
var
   ...
   // Override where libantimony is loaded from. Must be called BEFORE loadAntimonyLibrary.
   // Accepts a bare name (Windows DLL search) or a path relative to the executable.
   procedure setAntimonyLibraryName (const AName : string);
...
{$ELSE}
    if FLibHandle = 0 then
       begin
       errMsg := 'Antimony library could not be loaded from ' + path;
       Exit;   // leave result/DLLLoaded False; do not bind nil procs
       end;
{$ENDIF}
